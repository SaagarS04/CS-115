...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        '''This function take input string, result, curcount, and curval and using tail recursion
            finds and outputs the list of consecutive binary digits with the same value'''
        if (len(s) == 0):
            return result + [curCount]
        if (s[0] != str(curVal)):
            curVal = (int(curVal) + 1) % 2
            result += [curCount]
...
def findRunBits(s):
    ...
    '''This function takes input s and reuturns the optimal bit length based on the average run length'''
    averageRunLength = sum(listOfRunLengths(s))/len(listOfRunLengths(s))
    n = 1 + math.ceil(math.log(averageRunLength, 2))
    if (n > 7):
        return 7
    else:
        return n
